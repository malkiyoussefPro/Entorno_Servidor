...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $stmt_precio = $pdo->prepare("SELECT precio FROM servicios_hotel WHERE departamento = 'Belleza' AND descripción = ?");
        $stmt_precio->execute([$servicio]);
        $precio_servicio = $stmt_precio->fetchColumn();

        // Generar código único para la reserva
        $codigo_reserva = uniqid('BEL-');
        ...
        $stmt_reserva = $pdo->prepare("INSERT INTO reservas_servicios (nombre_cliente, email_cliente, telefono_cliente, departamento, servicio, hora_reserva, codigo_reserva, id_servicio, id_pago) VALUES (?, ?, ?, 'belleza', ?, ?, ?, ?, ?)");
