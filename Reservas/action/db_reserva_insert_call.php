...
if(isset($_SESSION['name'])) {
    ...
    // Definir la función obtenerIdCliente() solo si la sesión 'nombre_usuario' está configurada
    ...
    // Ejemplo de consulta para obtener el ID del cliente
    ...
    if($result) {
        $idCliente = $result['id_usuario'];
        echo "El ID del cliente asociado al nombre de usuario $nombreUsuario es: $idCliente";
    } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
    if (isset($_COOKIE['id_habitacion'], $_COOKIE['fecha_entrada'], $_COOKIE['fecha_salida'])) {
        if (!empty($_COOKIE['id_habitacion']) && !empty($_COOKIE['fecha_entrada']) && !empty($_COOKIE['fecha_salida'])) {
            ...
            if(isset($_COOKIE['id_usuario'])) {
                ...
                $_SESSION['id_usuario'] = $_COOKIE['id_usuario']; // Guarda el valor del cookie en la sesión si está disponible
                $id_habitacion = $_COOKIE['id_habitacion'];
                $fecha_entrada = $_COOKIE['fecha_entrada'];
                $fecha_salida = $_COOKIE['fecha_salida'];
            }
            ...
            $id_cliente = obtenerIdCliente($pdo);
